...
continuare ad oscillare tra $\pm \Delta$.
...
useremo sequenze di campioni $x[n] = x_q(n\ T_c)$.
...
- **cross-correlazione**:
  - **segnali di energia**:
    $\displaystyle \mathcal{R}_{x,y}(k) = \sum_{n = -\infty}^{+\infty} x[n]\ y[n + k]$
  - **segnali di potenza**:
    $\displaystyle \mathcal{R}_{x,y}(k) = \lim_{K \to \infty} \frac{1}{K} \sum_{n = -\frac{K}{2}}^{\frac{K}{2}} x[n]\ y[n + k]$
...
- invarianza temporale: $f(x[n]) = y[n] \implies f(x[n - k]) = y[n - k]$;
