...
$$
x(t)\ \delta(t - n T_c) = x(n T_c)\ \delta(t - n T_c)
$$

A questo punto basta ripetere l'operazione attraverso un treno di impulsi:

$$
x_c(t) = x(t) \sum_{n=-\infty}^{+\infty} \delta(t - n T_c)
$$
...
banda**, cioè con spettro nullo oltre la frequenza $f_\text{max}$ (un segnale a
...
$x(t)$ moltiplicato per $\frac{1}{T_c}$ e ripetuto con periodicità
$\frac{1}{T_c}$ sull'asse delle frequenze.
...
le repliche devono essere distanziate di almeno due volte $f_\text{max}$
($f_c \geq 2 f_\text{max}$).

Avvicinandosi al limite $f_c = 2 f_\text{max}$ si rischia comunque di cadere in
casi in cui non è possibile ricostruire il segnale. Considerando il
funzionamento di un filtro passa-basso reale che taglia le frequenze in modo
graduale, si consiglia anche di aggiungere una costante, detta **banda di
...
$H_\text{LPF}(f) = T_c \operatorname{rect}\left(\frac{f}{f_c}\right) \iff h_\text{LPF}(t) = \operatorname{sinc}\left(\frac{t}{T_c}\right)$.

Quindi il segnale ricostruito sarà:

$$
x(t) = \sum_{n=-\infty}^{+\infty} x(n T_c)\ \operatorname{sinc}\left(\frac{t - n T_c}{T_c}\right)
$$

Il picco di una singola $\operatorname{sinc}(t)$ dà il valore del segnale nel
punto di campionamento. Negli altri punti invece essa viene annullata dalle
$\operatorname{sinc}(t)$ degli istanti precedente e successivo.
...
dove $h_H(t)$ è una funzione rettangolare. La $H_H(f)$ sarà una
$\operatorname{sinc}(f)$, quindi si introduce necessariamente una distorsione
lineare che si può correggere con un altro filtro equalizzatore.
