...
Dati un segnale discreto $x[n]$, $n = 0, \ldots, N_x - 1$ e un filtro $h[n]$,
$n = 0, \ldots, N_h - 1$. La loro convoluzione sarà:
...
$$
Y[k] = X[k]\ H[k] \implies y[n] \overset{\mathcal{F}}{\longleftrightarrow} Y[k]
$$
...
$$
x[n] \circledast h[n] \overset{\mathcal{F}}{\longleftrightarrow} X[k]\ H[k]
$$
...
1. Generiamo 2 sequenze di lunghezza $M \geq N_x + N_h - 1$ aggiungendo gli zeri
...
$$
y(v_1, \ldots, v_N) = x(v_1, \ldots, v_N) * h(v_1, \ldots, v_N) = \int_{\lambda_1 = -\infty}^{+\infty} \dots \int_{\lambda_N = -\infty}^{+\infty} x(\lambda_1, \ldots, \lambda_N)\ h(v_1 - \lambda_1, \ldots, v_N - \lambda_N)\ d\lambda_N \dots d\lambda_1
$$
...
$$
X(f_1, \ldots, f_N) = \int_{-\infty}^{+\infty} \dots \int_{-\infty}^{+\infty} x(v_1, \ldots, v_N)\ e^{-j 2 \pi (f_1\, v_1 + \dots + f_N\, v_N)}\ \mathrm{d}v_1 \dots \mathrm{d}v_N
$$
