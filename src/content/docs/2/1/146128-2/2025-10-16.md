...
$$
y(t) = x(t) * h(t) \xleftrightarrow{\mathcal{F}} Y(f) = X(f)\ H(f)
$$
...
- **Risposta in fase** $\Phi(f) = \arg(H(f))$: dice quanto viene sfasata ogni
  componente del segnale.
- **Ritardo di gruppo** $\tau(f) = -\frac{1}{2\pi}\ \frac{d\Phi(f)}{df}$: dice
...
- **Sistemi in serie**:
  ...
  La risposta in frequenza è $H_{\text{tot}}(f) = H_1(f)\ H_2(f)$.
...
$G_{\mathrm{dB}} = 10\ \log_{10}(G_{\mathrm{W}}) = -3\ \mathrm{dB}$. Quindi
$G_{\mathrm{W}} = \frac{1}{2}$.
