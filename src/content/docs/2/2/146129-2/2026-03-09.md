---
description:
  Analisi delle regioni del condensatore MOS e funzionamento del transistore
  nMOS nei regimi lineare e di saturazione.
lang: it
title: Fondamenti del condensatore MOS e del transistore nMOS
---
...
- $V_{GS} \ll V_T$: i diodi sono polarizzati a zero o inversamente, quindi
...
- $V_{GS} > V_T$: il canale tra le due regioni $n+$ è costituito da elettroni
  ...
  $$
  Q' = - W\ C''_{ox}\ (v_{ox} - V_T)
  $$

  Dove:
  - $v_{ox} = V_{GS} - v(x)$, dove $v(x)$ è la tensione a distanza $x$ nel
    canale. $v_{ox}$ deve essere sempre maggiore di $V_T$.
  ...
  $$
  i(x) = - \mu_n\ W\ C''_{ox}\ (V_{GS} - v(x) - V_T)\ \frac{dv}{dx}(x)
  $$

  Si può integrare lungo $x$:

  $$
  \int_0^L i(x)\ dx = \int_0^{V_{DS}} - \mu_n\ W\ C''_{ox}\ (V_{GS} - v(x) - V_T)\ dv(x)
  $$
...
Se il MOSFET è acceso ($V_{GS} > V_{DS} + V_T$) e $V_{DS}$ è molto piccola,
...
scomparire quando $V_{GS} = V_{DS} + V_T$ (pinch-off point). Ulteriori aumenti
di $V_{DS}$ non fanno scorrere più corrente.
