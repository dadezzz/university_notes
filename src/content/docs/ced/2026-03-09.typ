...
- $V_"GS" < V_"TN"$: si forma la regione di svuotamento sotto il gate, che si
  estende fino a quelle dovute alle giunzioni $p n$, ma non scorre corrente
  perché la regione è costituita solo da ioni fissi;

- $V_"GS" > V_"TN"$: il canale tra le due regioni $n+$ è costituito da elettroni
  liberi, quindi se $V_"DS" != 0$ può scorrere corrente tra source e drain;

  All'interno del canale la tensione varia da $V_"S"$ a $V_"D"$ in modo
  ...
  - $C''_"ox" = epsilon_"ox" / T_"ox"$, dove $T_"ox"$ è lo spessore dello strato
    di biossido di silicio ($"SiO"_2$).

  La corrente si calcola con $i(x) = Q'(x) s_x(x)$, dove $s_x = - mu_n E_x$ è la
  velocità di deriva delle cariche dovuta al campo elettrico.
