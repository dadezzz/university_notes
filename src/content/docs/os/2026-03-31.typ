...
#metadata((
  description: "Explores the four necessary conditions for deadlock, resource allocation graphs, prevention and avoidance strategies, the Banker algorithm, and detection methods.",
  lang: "en",
...
- no preemption: a resource can be released only voluntarily by the thread
  holding it;
- circular wait: in a set of threads $\{T_0, ..., T_n\}$ of waiting threads,
  $T_0$ is waiting for a resource held by $T_1$, $T_1$ is waiting for $T_2$ and
  so on until $T_n$ is waiting for $T_0$.
...
*Problems*: aborting could leave the system in an inconsistent state, and it is
difficult to choose the right order in which threads should be aborted.
...
*Problems*: it is still difficult to choose the thread whose resources should be
preempted, as well as the risk of starvation, since a thread may always be
