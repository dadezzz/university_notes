...
#metadata((
  description: "Explores memory segmentation, demand paging, swapping, and page replacement algorithms including FIFO, LRU, and second chance for memory management.",
  lang: "en",
...
The *pager* is the component of the OS that decides which pages to load from the
disk and which ones to evict from memory.
...
- *Fixed allocation*: each process receives a predetermined share of the system
  memory that could be divided equally or proportionally to process size.
...
- *Second chance*: Another approximation of LRU that combines the reference bit
  with FIFO.
