...
#metadata((
  description: "Covers address binding, base-limit memory protection, paging, virtual memory and page table mechanisms for operating system memory management and allocation.",
  lang: "en",
...
1. *Fetch*: retrieve the instruction bytes from the memory address pointed to by
  the PC;
...
5. *Store result*: optionally, the result may need to be stored back to memory;
...
- *Compile time*: the memory location of the program is known before execution.
  The compiler generates absolute addresses. This is often used in bare-metal
  embedded systems.
...
- *internal fragmentation*: occurs when the allocated memory is slightly larger
  than requested (e.g., two partitions allocated, but the second is mostly
  unused);

== Paging

Paging is the subdivision of physical memory into fixed-size blocks called
*frames*. Logical memory is divided into blocks of the same size called *pages*.
