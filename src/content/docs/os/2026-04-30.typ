...
#metadata((
  description: "File system operations, directory structures, access control mechanisms, and memory-mapped files for efficient storage management in operating systems.",
  lang: "en",
...
#starlight.note(title: "Directories", [
...
- *Write*: the user must specify the file handle and data to write. The OS keeps
  a pointer to the location where the next write shall take place.
- *Read*: the user must specify the file handle and the location in memory where
  to put the read data. The OS keeps a pointer (the same as the write one) to
  the location where the next read shall take place.
- *Seek*: reposition the read/write pointer within the file. No I/O is involved.
...
- *Sequential access*: this is the simplest method. The supported operations are
  `read next`, `write next` and `reset` (the pointer to the beginning of the
...
- *Single level directory*: all files are contained in the same directory. There
