...
- *Contiguous allocation*: each file occupies a set of contiguous blocks on the
  device. This has some performance benefits on HDDs, where sequential reads are
  much faster than random ones.
...
- *Linked allocation*: in linked allocation, each file is a linked list of data
  blocks. Each block contains a pointer to the location of the successive one.
...
- *Combined scheme*: An inode contains a fixed number of pointers. Some point to
  ...
  of indirection for each pointer).
...
- *Linked free space list*: each free block has a pointer to the next free
  ...
  - *Counting*: since free space is often found in contiguous blocks, we keep a
    count of them and a pointer to the next cluster, stored only on the first
    block of the group.
...
*Consistency checking* (`fsck` on UNIX) compares data in the directory structure
with data blocks on disk and attempts to fix inconsistencies. This method is
slow and can sometimes fail, so it is used only as a last resort.
