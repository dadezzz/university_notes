...
#metadata((
  description: "Disk block allocation strategies, free space tracking, journaling crash recovery, mount procedures, and virtual file system abstraction for multiple filesystems.",
  lang: "en",
...
- *Contiguous allocation*: each file occupies a set of contiguous blocks on
  ...
  #image("images/disk-alloc-contiguous.png", alt: "Contiguous allocation on disk")
...
- *Combined scheme*: An inode contains a fixed number of pointers. Some point
  ...
  #image("images/disk-alloc-combined-scheme.png", alt: "Combined scheme on disk")
...
- *Bit vector*: each block is represented by one bit. A value of 1 means the
  ...
  To find free space, we just need to scan the whole vector. This works well for contiguous allocation, since finding contiguous zero bits is fast.

- *Linked free space list*: each free block has a pointer to the next free block.

  It is efficient when the number of free blocks is known, but it is difficult to locate contiguous space easily. This can be partially solved with:
  - *Grouping*: each list entry holds a group of pointers to free blocks.
  - *Counting*: since free space is often found in contiguous blocks, we keep a count of them and a pointer to the next cluster, stored only on the first block of the group.
