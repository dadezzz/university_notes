#import "../_templates/starlight.typ" as starlight

#show: starlight.setup

#metadata((
  description: "Deriva l'integrale di convoluzione dalla risposta all'impulso nei sistemi LTI, con linearità, invarianza temporale, interpretazione grafica e proprietà",
  lang: "it",
  title: "Integrale di convoluzione per sistemi LTI",
))

= Sistema analogico LTI

Un sistema di elaborazione di segnali è costituito in genere da una cascata di
blocchi che applicano una trasformazione al segnale.

I sistemi che studieremo in questo corso saranno lineari e tempo-invarianti.

== Linearità

La linearità viene espressa matematicamente come segue: dati 2 segnali $x_1(t)$
e $x_2(t)$ e 2 numeri reali $a$ e $b$, un sistema $f(x)$ è lineare se:

$
  f(a x_1(t) + b x_2(t)) = a f(x_1(t)) + b f(x_2(t))
$

Nei sistemi lineari vale il principio di sovrapposizione degli effetti: la
risposta a una combinazione lineare di ingressi è la combinazione lineare delle
rispettive risposte.

== Invarianza temporale

In un sistema tempo-invariante, la risposta non cambia al variare del ritardo
del segnale in ingresso.

$
  f(x(t)) = y(t) => f(x(t - t_0)) = y(t - t_0)
$

= Convoluzione

Dato un sistema lineare tempo-invariante, è possibile calcolare la risposta ad
un segnale in ingresso $x(t)$ qualsiasi a partire dalla conoscenza della
risposta all'impulso unitario $delta(t)$.

Quindi ogni sistema può essere caratterizzato in base alla sua risposta a
$delta(t)$.

*Dimostrazione*:

1. Prendiamo un segnale $x(t)$ e lo approssimiamo tramite una serie di
  rettangoli di durata $Delta T$ e area unitaria.

  Chiameremo $x_(R)(t)$ il segnale approssimato e $R(t)$ ogni rettangolo.
  Inoltre $h_(R)(t)$ sarà la risposta del sistema a ogni rettangolo.

  #starlight.img(
    "images/suddivisione-segnale-rettangoli.png",
    alt: "Suddivisione del segnale in rettangoli",
  )

2. Poiché il sistema è LTI:
  - la risposta ad una somma pesata di ingressi $R(t)$ è la somma delle uscite
    $h_(R)(t)$;
  - la traslazione di $R(t)$ produce una pari traslazione della corrispondente
    uscita;

  Quindi la risposta al segnale $x_(R)(t)$ è:

  $
    y_(R)(t) = sum_k x(k Delta T) h_(R)(t - k Delta T) Delta T tilde.eq y(t)
  $

3. Usando un $Delta T$ più piccolo miglioreremo l'approssimazione (l'area
  dell'errore tende a 0).

  Al limite $Delta T -> 0$ la funzione $R(t)$ tende ad una funzione di durata
  nulla, ampiezza infinita e area unitaria. Abbiamo definito questa funzione
  come $delta(t)$.

  La risposta al rettangolo ($h_(R)(t)$) tende a sua volta alla risposta
  all'impulso (che chiameremo $h(t)$).

  $x_(R)(t)$ diventa l'integrale di una serie infinita di impulsi, coincidente
  con $x(t)$:

  $
    lim_(Delta T -> 0) x_(R)(t) = integral_(-oo)^(+oo) x(tau) delta(t - tau) dif tau = x(t)
  $

4. La stessa cosa vale per la risposta del sistema:

  $
    lim_(Delta T -> 0) y_(R)(t) = integral_(-oo)^(+oo) x(tau) h(t - tau) dif tau = y(t) = x(t) * h(t)
  $

  L'integrale sopra definito prende il nome di *integrale di convoluzione* e
  rappresenta l'uscita esatta $y(t)$ di un sistema LTI in corrispondenza di un
  generico ingresso $x(t)$.

#starlight.caution([
  Non è detto che l'integrale di convoluzione sia facilmente risolvibile.
])

== Interpretazione grafica

Dato un segnale $u(t)$ e la risposta impulsiva $h(t)$, la loro convoluzione
$y(t) = u(t) * h(t)$ può essere interpretata come la sequenza di 3 operazioni:

1. ribaltamento di una delle due funzioni;
2. traslazione della funzione ribaltata;
3. prodotto punto a punto e integrazione del prodotto.

#starlight.img(
  "images/interpretazione-grafica-convoluzione.png",
  alt: "Interpretazione grafica della convoluzione",
)

== Proprietà

- restituisce una funzione che ha durata maggiore di quella degli operandi;
- è un'operazione lineare (perché è un integrale);
- gode delle proprietà commutativa, associativa e distributiva rispetto alla
  somma.
